...
type BuildFunction = (
  world: World,
  buildStep: BuildTaskI
) => Promise<BuildOrderStatus>;
...
interface PlayerData
  extends SC2APIProtocol.PlayerCommon,
...
interface UnitResource {
  ...
  getClosest(
    pos: Point2D,
    units: SC2APIProtocol.PowerSource[],
    n?: number
...
interface ActionManager {
  ...
  attack(
    ...
    unit?: Unit,
    queue?: boolean
  ): Promise<SC2APIProtocol.ResponseAction>;
  attackMove(
    u?: Unit[],
    p?: Point2D,
    queue?: boolean
  ...
  patrol(
    unit: Unit,
    positions: Array<[Point2D, Point2D]>,
    queue?: boolean
  ): Promise<SC2APIProtocol.ResponseAction>;
  patrol(
    unit: Unit[],
  ...
  sendQuery: (
    query: SC2APIProtocol.RequestQuery
  ) => Promise<SC2APIProtocol.ResponseQuery>;
}
...
type EventConsumer = {
  ...
  onChatReceived?: (
    world: World,
    chat: SC2APIProtocol.ChatReceived
  ) => Promise<any>;
  ...
  onExpiredEffect?: (
    world: World,
    effect: SC2APIProtocol.Effect & SC2APIProtocol.EffectData
  ) => Promise<any>;
};
...
interface Engine {
  ...
  runGame: (
    map: string,
    players: Array<{ type: number; race: number; agent?: Agent }>
  ) => Promise<GameResult>;
  createGame: (
    map: string,
    playerSetup: SC2APIProtocol.PlayerSetup[],
    realtime?: boolean
  ) => Promise<SC2APIProtocol.ResponseCreateGame>;
