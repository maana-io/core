...
const executeInfoText = fs
  .readFileSync(path.join(HOME_DIR, EXECUTE_INFO_PATH))
  .toString();
const executablePath = executeInfoText.match(
  /^\s*([\w.-]+)\s*=\s*(.*)?\s*$/m
)[2];
...
const basePath = parsedPath
  .slice(0, parsedPath.findIndex(s => s === "StarCraft II") + 1)
  .join(path.sep);
...
async function launcher(options = {}) {
  ...
  if (samePortProcess && samePortProcess.pid !== 0) {
    // could show up as pid 0 for an old TIME_WAIT process
    ...
    if (options.force) {
      ...
      debug(
        "Skipping launch since the client already exists listening on this port (set { force: true } in options if you want to force restart it)"
      );
  ...
  if (existingProcesses.length > 0) {
    debug(
      `Existing ${execName} processes found running: %o`,
      existingProcesses
    );

    if (options.forceAll) {
      debug(
        `ForceAll" enabled, killing *all* ${execName} processes before launching...`
      );
      ...
      debug(
        "Not killing existing clients running on a different port, (set { forceAll: true } in options if you want this behavior)"
      );
    }
  } else {
  ...
  const clientProcess = spawn(
    ...
    [
      ...
      String(opts.port),
      "-displayMode",
      String(opts.displayMode)
    ],
    {
      cwd:
        os.platform() === "win32"
          ? path.join(basePath, "Support64")
          : undefined,
      detached: true,
      stdio: "ignore"
    }
  ...
  while (
    { pid: 0, ...(await findP("port", opts.port))[0] }.pid !== clientProcess.pid
  );
...
async function findMap(mapName) {
  ...
  const tryPath2 = path.join(
    basePath,
    MAP_DIR,
    mapName.replace(/\s/g, "") + SUFFIX
  );
