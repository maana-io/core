...
function createEngine(options = {}) {
  ...
  const engine = {
    ...
    async createGame(map = "", playerSetup = [], realtime = false) {
      if (isManaged) {
        debugEngine(
          `Engine tried to create a game, but is being managed by a ladder manager - skipping...`
        );
        return;
      }
      ...
      if (_client.status !== 1 && _client.status !== 5) {
        console.warn(
          `cannot create a game unless in "launched" or "ended" status, current status is ${
            StatusId[_client.status]
          }`
        );
        return;
      }
    ...
    async joinGame(agent, options = {}) {
      ...
      if (!agent.interface.raw) {
        throw new Error(
          "`raw` is currently the only interface supported by this engine."
        );
      }
      ...
      switch (_client.status) {
        ...
        case 3: {
          console.warn(
            `cannot join lobby for a game already in progress, attemping to resync to existing game...`
          );
        ...
        default: {
          throw new Error(
            `cannot join a game this engine didn't create unless in "launched" status, current status is ${
              StatusId[_client.status]
            }`
          );
        }
    ...
    async firstRun() {
      ...
      ["abilities", "units", "upgrades", "buffs", "effects"].forEach(
        dataType => {
          if (gameData[dataType]) data.set(dataType, gameData[dataType]);
        }
      );

      [...this.systems, ...agent.systems, agent, debugSystem].forEach(system =>
        system.setup(world)
      );
    ...
    onGameEnd(results) {
      ...
      debugEngine(
        `Average step delay for this game: ${this._totalLoopDelay /
          world.resources.get().frame.getGameLoop()}`
      );
    ...
    async runLoop() {
      ...
      if (this._lastRequest) {
        this._loopDelay = hrtimeH(
          process.hrtime(this._lastRequest)
        ).milliseconds;
      ...
      try {
        ...
        if (e instanceof GameEndedError) {
          ...
          console.warn(
            "An error of an unknown type has propegated up to the game loop",
            e
          );
          throw e;
        }
    ...
    async dispatch() {
      ...
      const { events } = resources.get();
      const eventsWithHandlers = events
        .read(agent.readerId)
        .filter(event => agent[`on${pascalCase(event.name)}`]);
      const handlerPromises =
        eventsWithHandlers.length > 0
          ? eventsWithHandlers.map(event => {
              return agent[`on${pascalCase(event.name)}`](
                world,
                event.data,
                event
              ).catch(err => {
                if (err instanceof NodeSC2Error) {
                  debugSilly(
                    `Agent handler error: on${pascalCase(event.name)}: `,
                    err
                  );
                } else {
                  console.warn(
  ...
  engine.use([frameSystem, unitSystem, mapSystem]);
